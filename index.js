...
function customAssign(...objects) {
  ...
  if (
    !(typeof objects === "object" && objects !== null) ||
    objects.length < 2
  ) {
...
function customObjectFromEntries(entries) {
  ...
  const entriesMapFrom = Object.fromEntries(entriesMap); // Використання методу `fromEntries` для створення об'єкта з масиву записів
  return entriesMapFrom; // Повернення створеного об'єкта
}
...
function filterObjectsByKey(arr, key) {
  ...
  const arr1 = arr.filter((obj) => obj.hasOwnProperty(key));
  return arr1; // Повертаємо новий масив з об'єктами, що містять вказаний ключ
}
